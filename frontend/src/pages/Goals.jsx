    ...
    <div className={`card ${isCompleted ? 'border-2 border-primary-300 dark:border-primary-700' : ''}`}>
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => onDeleteRequest(goal.id)} className="text-gray-400 hover:text-red-500 dark:hover:text-red-400 transition-colors">
            <Trash2 size={14} />
          </button>
        </div>
      </div>
          ...
          {showAdd ? (
            <form onSubmit={handleAdd} className="flex gap-2">
              <input type="number" step="0.01" min="0.01" max={remaining}
                ...
                placeholder={`Até ${fmt(remaining)}`} className="input-field text-sm flex-1" required />
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
                  ...
                  {emojis.map((e) => (
                    <button key={e} type="button" onClick={() => setForm({ ...form, emoji: e })}
                      className={`text-xl w-9 h-9 rounded-lg flex items-center justify-center transition-all ${
                        form.emoji === e ? 'bg-primary-100 dark:bg-primary-900/40 ring-2 ring-primary-500' : 'hover:bg-gray-100 dark:hover:bg-gray-800'
                      }`}>{e}</button>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                  className="input-field" placeholder="Ex: Fundo de emergência, Viagem..." required />
              </div>
