...
function GoalCard({ goal, onEdit, onDeleteRequest, onUpdateProgress }) {
  ...
  return (
    <div className={`card ${isCompleted ? 'border-2 border-primary-300 dark:border-primary-700' : ''}`}>
      ...
      <div className="flex items-start justify-between mb-3 gap-2">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            ...
            {goal.notes && (
              <p className="text-xs text-gray-400 dark:text-gray-500 truncate">{goal.notes}</p>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          {isCompleted ? (
            <span className="flex items-center gap-1 text-xs text-primary-600 dark:text-primary-400 font-semibold whitespace-nowrap">
              <Trophy size={12} />
      ...
      {/* valores */}
      <div className="flex items-center justify-between text-xs sm:text-sm mb-3 gap-1">
        <span className="text-gray-500 dark:text-gray-400">
          Guardado:{' '}
          <span className="font-semibold text-gray-900 dark:text-gray-100">{fmt(goal.current_amount)}</span>
        </span>
        <span className="text-gray-500 dark:text-gray-400 hidden sm:inline">
          Meta:{' '}
          <span className="font-semibold text-gray-900 dark:text-gray-100">{fmt(goal.target_amount)}</span>
        </span>
        <span className="font-bold text-blue-600 dark:text-blue-400 flex-shrink-0">
          {pct.toFixed(0)}% <span className="sm:hidden text-gray-400 dark:text-gray-500 font-normal">de {fmt(goal.target_amount)}</span>
        </span>
      </div>
      ...
      {isCompleted && (
        <div className="flex items-center justify-center gap-2 py-2 text-primary-600 dark:text-primary-400 text-sm font-semibold">
          <CheckCircle2 size={15} /> Meta alcançada! Parabéns!
        </div>
      )}
...
export default function Goals() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editGoal) await goalsAPI.update(editGoal.id, payload)
  ...
  return (
    <div className="p-4 sm:p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-4 sm:mb-6 gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100">Metas de Economia</h1>
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" step="0.01" min="1" value={form.target_amount}
                    onChange={(e) => setForm({ ...form, target_amount: e.target.value })}
                    className="input-field" placeholder="2.000,00" required />
                </div>
                <div>
              ...
              <div>
                ...
                <input value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })}
                  className="input-field" placeholder="Contexto ou motivação..." />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                {active.map((goal) => (
                  <GoalCard key={goal.id} goal={goal} onEdit={openEdit}
                    onDeleteRequest={setDeleteId} onUpdateProgress={handleUpdateProgress} />
                ))}
              </div>
            </div>
          )}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl p-6 w-full max-w-sm">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="font-semibold text-gray-900 dark:text-gray-100">Remover meta?</h3>
            </div>
